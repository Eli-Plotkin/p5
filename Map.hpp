...
private:
  ...
  class PairComp {
    public:
      bool operator()(const Pair_type x, const Pair_type y) {
        Key_compare less;
...
public:
  ...
  Value_type& operator[](const Key_type& k) {
    ...
    if (finding == BST.end()) {
      Iterator newFind = BST.insert({k, Value_type()});
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val)  {
    Iterator result = find(val.first);
    if (result == BST.end())  {
      Iterator newResult = BST.insert(val);
