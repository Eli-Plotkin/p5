...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    //empty tree 
    if (!node)  {
      Node * newNode1 = new Node{item, nullptr, nullptr};
      return newNode1;
    }

    else if (less(item, node->datum))  {
      if (!node->left)  {
        Node * newNode2 = new Node{item, nullptr, nullptr};
        node->left = newNode2;
      }
      else {
      ...
      if (!node->right) {
        Node * newNode3 = new Node{item, nullptr, nullptr};
        node->right = newNode3;
      } 
      else {
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    //node is less than val
    else if (less(node->datum, val))  {
      //no element thats greater in tree
      if (!node->right) {
        return nullptr;
      }
      //greater element is on right side
      return min_greater_than_impl(node->right, val, less);
    }
    else if (less(val, node->datum)) {
      if ((!node->left) || less(node->left->datum, val) || 
      (!less(node->left->datum, val) && !less(val, node->left->datum))) {
        return node;
      }
      else  {
